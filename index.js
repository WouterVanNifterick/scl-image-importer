...
const walk = async (dir, ext, level) => {
    ...
    {
        insertSQL = `INSERT IGNORE INTO scl_image (scl_imagesetid, filename, timestamp, lp, country, len, conf, topleftx, toplefty, toprightx, toprighty, bottomrightx, bottomrighty, bottomleftx, bottomlefty, classifier_rdwclass) VALUES ` + insertSQL.join(", \n");
    ...
    if(level===0){
        ...
        fs.writeFile(startDir+"/insert.sql", sql, e=>{console.error(e||"Save OK"); 
...
function main(){
    var dbConn = newConnection(); 
    dbConn.query('insert ignore into scl_imageset(scl_imageset,basepath) values (?,?)', [startDir,startDir], function (error, results, fields) {
        console.error(error||"Insert OK");
        imagesSetId = results?results.insertId:5;
        dbConn.commit( error=>{
            console.error(error||"Commit OK");
            dbConn.end(e=>{console.error(e||"Destruct conn OK"); 
